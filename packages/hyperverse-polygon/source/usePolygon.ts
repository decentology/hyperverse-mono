...
function PolygonState() {
	...
	const infuraNetwork = network === networks.Mainnet ? 'mainnet' : 'mumbai';
	...
	const connect = useCallback(async function () {
		try {
			...
			if (blockchain?.name === blockchains.Polygon && userNetwork.chainId !== 4) {
	...
	useEffect(() => {
		if (blockchain?.name === blockchains.Polygon) {
...
const Polygon = createContainer(PolygonState);

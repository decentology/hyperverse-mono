...
describe('SafuuX', function () {
	...
	it('Whitelist has a ETH blanace', async function () {
		...
		expect(
			Number(await this.safuuToken.balanceOf(this.WHITE_LIST[0].address))
	...
	it('Should NOT mint from Public Sale while GoldList is active only', async function () {
		await expect(this.safuux.mintLiteNode(2)).to.be.revertedWith('Public sale not active');
	});

	it('Should enable Public sale mint', async function () {
		expect(await this.safuux._isPublicMintActive()).to.equal(false);
		const saleStatus = this.safuux.setPublicMintStatus(true);
		expect(await this.safuux._isPublicMintActive()).to.equal(true);
	});
	...
	it('Should mint from Public Mint', async function () {
		//Check balance before mint
		const fullNodeBalBefore = await this.safuux.balanceOf(this.WHITE_LIST[0].address, 1);
		const liteNodeBalBefore = await this.safuux.balanceOf(this.WHITE_LIST[0].address, 2);
		...
		//Check balance after mint
		const fullNodeBalAfter = await this.safuux.balanceOf(this.WHITE_LIST[0].address, 1);
		const liteNodeBalAfter = await this.safuux.balanceOf(this.WHITE_LIST[0].address, 2);
	...
	it('Should burn tokens', async function () {
		const currentLiteNodeBalance = await this.safuux.balanceOf(this.accounts[0].address, 2);

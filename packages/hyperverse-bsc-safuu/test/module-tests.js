...
describe('SafuuX', function () {
	...
	it('Should mint from GoldList', async function () {
		...
		const tx = await this.safuux.mintFullNode(
			1,
		...
		expect(liteNodeBalAfter.toNumber()).to.equal(0);
	});

	it('Should NOT be able to mint more than 1 Full Node', async function () {
		await expect(
			this.safuux.mintFullNode(1, generateMerkleProof(this.GOLD_LIST, this.GOLD_LIST[0]))
	...
	describe('Mint', () => {
		...
		it('Should validate merklee proof', async function () {
			await expect(
				this.safuux
					.connect(this.WHITE_LIST[0])
					.mintFullNode(1, generateMerkleProof(this.GOLD_LIST, this.GOLD_LIST[0]))
			).to.be.revertedWith('Address not eligible - Invalid merkle proof');
		});
	});
	...
	it('Should mint from WhiteList', async function () {
		...
		const tx = await this.safuux
			.connect(this.WHITE_LIST[0])
			.mintFullNode(1, generateMerkleProof(this.WHITE_LIST, this.WHITE_LIST[0]));
	...
	describe('Burn', function () {
		beforeEach(async function () {
			...
			await this.safuux.mintLiteNode(2);
		});

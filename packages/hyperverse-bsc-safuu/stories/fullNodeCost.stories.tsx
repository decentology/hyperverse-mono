...
const Component = ({ cost }: { cost: number }) => {
	const { setFullNodeCost, getFullNodeCost, getFullNodeSupply, getFullNodeLimit } = useSafuu();
	...
	useEffect(() => {
		getFullNodeSupply?.().then(setLocalSupply);
	}, [getFullNodeSupply]);
	useEffect(() => {
		getFullNodeLimit?.().then(setLocalLimit);
	}, [getFullNodeLimit]);
	const handleCost = () => {

...
<div className="body">
	Check out the{' '}
	<a href="https://storybook.js.org/docs/react/get-started/whats-a-story">Storybook Addons</a>{' '}
	section for a more comprehensive guide to writing stories.
</div>
...
<div className="subheading">getBalance.ts</div>

<div className="body">
	Before we begin writing our story, we'll take a look at the <b>getBalance</b> function from
	<b>getBalance.ts</b> to get a better idea of what we'll need. We can see that this function needs
	a <b>tenantId</b> and an <b>account</b> in order to return a number for the balance.
</div>
...
async function getBalance(tenantId: string, account: string) {
	...
	try {
      ...
      `,
				fcl.args([fcl.arg(tenantId, t.Address), fcl.arg(account, t.Address)]),
			])
			.then(fcl.decode);
...
<div className="subheading">getBalance.stories.tsx</div>
...
<div className="body">
	...
	will call the <b>GetBalance</b> component. It is binded with our first story we have titled{' '}
	<b>Demo</b>. We provide an initial tenant Id of{' '}
	<b>0xf39fd6e51aad88f6f4ce6ab8827279cfffb92266</b> and an account of
	<b>0xf39fd6e51aad88f6f4ce6ab8827279cfffb92266</b> that we will pass through as the props <b>
		getBalance
	</b> requires.
</div>
...
<div className="body">
	...
	specific states. For example, you could write another story with an initial address that does
	not have a tribe Id associated with it.
</div>
...
<div className="body">
	...
	interactions we want to have with the <b>getBalance</b> function.
</div>

...
function TokenState(initialState: { tenantId: string } = { tenantId: TENANT_ADDRESS }) {
	...
	const { address, web3Provider, provider } = usePolygon();
	...
	return {
		...
		NewInstance: (
			...
			useMutation(
				({ account, name, symbol, decimal }:{account: string, name: string, symbol: string, decimal: number}) =>
		...
		Transfer: (
			options?: Omit<
				...
				'mutationFn'
			>
		) => useMutation(({ to, value }:{to: string, value: number}) => transfer(to, value), options),
		TransferFrom: (
			options?: Omit<
				UseMutationOptions<
					unknown,
				...
				'mutationFn'
			>
		) => useMutation(({ from, to, value }:{from:string, to: string, value: number}) => transferFrom(from, to, value), options),
		...
		Mint: (
			options?: Omit<
				...
				'mutationFn'
			>
		) => useMutation(({ amount }:{amount: number}) => mint(amount), options),
		...
		Burn: (
			options?: Omit<
				...
				'mutationFn'
			>
		) => useMutation(({ amount }:{amount:number}) => burn(amount), options),
		...
		Approve: (
			options?: Omit<
				...
				'mutationFn'
			>
		) => useMutation(({ spender, amount }:{spender: string, amount: number}) => approve(spender, amount), options),
	};
}
